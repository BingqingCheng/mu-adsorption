...
def read_lammpstrj(filedesc):
    ...
    for i in range(3): 
        [cellmin, cellmax] = filedesc.readline().split()[:2]
...
def Sk(names, q, kgrid, e_A):
    # This is the un-normalized FT for the density fluctuations
    q_A = np.asarray([ q_now for i,q_now in enumerate(q) if names[i]==e_A ])
    #print(q_A)
    n_A = len(q_A)
...
def main(sprefix="Sk", straj="mW", sbins=24):
    ...
    # Outputs
    ofile_AA = open(sprefix+'-AA-real.dat',"ab")
    ...
    while True:
        ...
        if (nframe == 1):
            ...
            kgrid = np.zeros((bins,3),float)
            kgridbase = np.zeros((bins,3),float)
            # initialize k grid
            [ dkx, dky, dkz ] = [ 1./cell[0], 1./cell[1], 1./cell[2] ]
            for k in range(1,bins):
                # initialize k grid
                kgridbase[k,:] = (2.*pi)*np.array([0, 0, k])
                kgrid[k,:] = [ 0, 0, dkz*k ]
        ...
        sk_AA = Sk(names, sq, kgridbase, 'mW')
